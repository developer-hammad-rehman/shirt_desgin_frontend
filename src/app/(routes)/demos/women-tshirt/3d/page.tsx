...
import { womenTshirtColor } from '@/data/models';
import Spline from '@splinetool/react-spline'
import Link from 'next/link';
import React, { useState } from 'react'

export default function Customize_3d() {
  ...
  return (
    <div className='w-full flex flex-wrap lg:flex-nowrap   bg-[#2D2E32] h-full lg:h-screen'>
       ...
       <h4 className="text-2xl font-bold">Select Your Pattern</h4>
         ...
         <button className='bg-pink-600 rounded-full p-3' onClick={() => setColor("pink")}/>
         <button className='bg-blue-500 rounded-full p-3' onClick={() => setColor("blue")}/>
         <button className='bg-teal-400 rounded-full p-3' onClick={() => setColor("segreen")}/>
         </div>
         <Link href={'/demos/women-tshirt/3d/print-3d'} className='text-xl font-bold p-5 bg-black rounded-xl'>Next</Link>
       </div>
       <div className='w-full lg:w-1/2 py-8'>
        {/* @ts-ignore */}
         <Spline scene={womenTshirtColor[color]}/>
       </div>
    </div>
  )
}
